...
**Card detail**: `src/ui/detail.ts` builds the rows for a car or a mod as pure data, tested; `src/ui/CardDetail.tsx` holds the dialog and its provider, `src/ui/detailContext.ts` the context that hands cards an opener; `src/ui/useDetail.ts` is the hook the two card components read, null where no panel is mounted so the cards render as before.
