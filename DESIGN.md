...
They are **loaners** (phase 28): always raceable and never owned. Their cards are not in the collection, they cannot be edited or broken, and a loaner car can be neither won nor lost in Pink Slips Mode. What a fresh collection owns is the smaller intro set in section 12, and the two moved apart so the garages could stay broad enough to teach the game while the collection stayed small enough to leave something to find. Before phase 28 the union of these three garages was the free grant.

**Random garages** (phase 44). A fourth way to race, beside the three loaners and whatever the player built: the game deals five cars from the whole roster and a thirty-card deck from every mod those cars can use. Like a loaner it is **always raceable and never owned**, and for the same reason it can never play Pink Slips Mode — its cars are ordinary roster cars, so a win would write cars into a collection that never opened them while the other side, owning none of it, lost nothing. It is dealt to **both** sides locally, so the match is symmetric, and to your seat alone online, where the other seat is a person who picks for themselves. Dealing again gives a fresh pair. It earns packs exactly as any other garage does, since a pack is for finishing a match and not for what was raced, and it cannot enter the rated queue, since a rating measures the garage a player built as well as how they play.
...
- Race screen: two lanes viewed from above, a car marker per lane advancing toward a finish line, distance in feet under each. Markers slide. The one other piece of motion is the race-end moment: the track holds at the finishing positions, the winner's finish line flashes, and a banner names the winner and shows the captured car as a pink slip with the tally, and one line on what taking it means: in a match played for nothing the car is out for the rest of the match, and in Pink Slips Mode it changes hands for real at the end unless it is a loaner car or a keepsake (backlog U38). It stays until Continue and comes before staging, the hotseat hand-over, and the result screen. Reduced-motion settings keep the banner and drop the motion.
...
| Accent | pink `#ff5ca8` for pink slips, prompts, and targets; gold `#f2c14e` for the staged car, fuel, and the distance on the track. A label on anything filled pink is asphalt `#17171a`, and a pink button lightens to `#ff7db9` under the pointer (backlog A12) |
| Type borders | Sports `#d7263d`, Luxury `#7a3e9d`, Muscle `#e8731c`, JDM `#1e6fd9`, EV `#1fa37a`, Off-road `#8c6b2f` |
| Mod families | Part `#6b7f99`, Boost `#2e9e5b`, Sabotage `#c0392b` for borders; as text on cream and behind a chip's white text, darkened toward the ink: Part `#5b6879`, Boost `#2d7244`, Sabotage `#aa3528` (backlog A12) |
...
**Start the match** (backlog U46): the start screen's one decision is held at the bottom of the screen, on a clear strip that lets taps through to the cards under it, until the page scrolls down to its place above the ways off the screen, where it settles. Below the two garage pickers it sat about three phone screens down and two on a desktop, so a first visit met Play the CPU, a mode already on, whose tap did nothing.

**Readable labels** (backlog A12, A13): every label meets 4.5:1 against the colour it sits on, and every control is at least 24px on a phone. White on the accent pink was 2.9:1, so the main button of every panel and the pink badge on a card take the table's asphalt instead; the family line on a mod card and the Part chips take the darker family inks in the palette; the track's ruler is 65% gold rather than 55%. `src/ui/contrast.test.ts` reads the fixed pairs from the stylesheet. A card's info button is 1.5rem, 24px on a phone, up from 22px.

**Pink Slips Mode toggle**: shown on the online screen only, to a signed-in player above the ranked and friend sections, with one line on what it means and a note that both players need it on. It is greyed with its reason when the seat is racing a garage the game dealt. CPU setup has no such toggle (backlog U44). The result screen's Pink Slips Mode block follows the packs line.
...
- Finishing a match against the CPU or an online opponent earns 1 pack; winning it earns 2. A hotseat match earns 1. Packs wait in a stack until opened, either from the pop-up that follows the winner banner and the result screen right after a match, or from the Collection screen, which shows every card, owned ones in color with their counts, unowned ones dimmed. A Cards filter of All, Owned and Not owned sits over both grids there and in the deck builder, beside Type, Tier and Family, and a filter that leaves nothing says so (backlog U49): a new collection holds 8 of 126 cars, and either grid was otherwise a long scroll of dimmed cards on a phone.
...
**Sending the room link** (backlog U47). On a touch screen with a share sheet the room's button is Share link and opens the sheet with the link and one line, since that is how a link reaches a friend on a phone; closing the sheet says nothing. Anywhere else it is Copy link, and a sheet that fails copies instead. When neither works the screen says so under the button and how to copy the link by hand, and one tap on the link selects all of it; the button used to stay Copy link, which read as done. The recovery code's Copy says the same when the browser refuses, and tells the player to write the code down, since it is shown once.
...
The accounts live in one Durable Object, `AccountDirectory`, behind a platform-free `Directory` class in `src/server/directory.ts`: one record per account with its provider identity, display name, rating, record, collection, saved garages, and sessions. A signed-in browser keeps a mirror of the account's collection and garages in `localStorage`, so every screen reads as before, and asks the service whenever something changes: the builder pushes garages with `PUT /me/garages`, the collection and the pack pop-up open packs with `POST /me/packs/open`, and a finished CPU or hotseat match reports itself with `POST /me/cpu-result`, which the service honours at most once a minute. Guest data is claimed once, on the first sign-in from a browser: card counts take the larger of the two copies, packs and credits add up, laps take the larger count, garages are kept, and the account is marked claimed so nothing is ever merged twice. The service cannot see a guest's history, so a claim is first held to what a real guest collection could hold (`capClaim` in `src/collection/collection.ts`, `TUNABLES.claim`): each card at most the copies a deck can use plus nine spares, finishes no more than the copies they sit on, keepsakes no more than the laps, at most 40 packs and three laps, and the credits, counting what the spares would scrap for, within 400, cut first and then the dearest spares, so spares claimed now cannot be scrapped past the budget later. A card that is not a card is dropped. A claim can still name one of every car, since a real guest can own them all; that is as far as a browser's word can be checked. Signing out takes the mirror with the session, so the browser is left with a guest on the intro set: the cards stay with the player and come back at the next sign-in, and a player made next on that browser cannot claim the last one's collection over again (backlog S25). Beyond a claim, which adds at most 40, nothing a client sends can set a pack count, and nothing can set a rating.
...
**Rematch** (phase 26). After a friend match played outside Pink Slips Mode, the result screen's Play again asks the room for a rematch. The room tells both seats who has asked, so the screen reads "Waiting for X" on the seat that asked and "X wants to play again." on the other. When both have asked, the room starts a new match in the same room: the same garages and names, the seed advanced by one per match, the first move given to the seat that did not have it (the engine's `MatchConfig.firstPlayer`, which skips the coin flip), the take-back stack and the result latch cleared, and both seats get fresh views, so the board returns without leaving the room. The new match is reported at its end like any other. A ranked room refuses, since Play again there queues again, and a Pink Slips Mode room refuses, since its cars changed hands and the same garages would race a car the loser no longer owns; both keep New room. The saved seat now outlives the result, so a refresh during the offer rejoins the room; Leave, New room, and Back to start clear it.
...
**Pink Slips Mode online** (phase 20). Both seats must be signed in with it on. The ranked queue carries each player's toggle and pairs a Pink Slips Mode player only with another; the room the queue sets up is marked for it. In a friend room the first player to join sets the room's setting from their toggle; a joiner whose toggle differs is refused with a reason that names it, and a guest can neither open nor join a Pink Slips Mode room. When the match ends the room computes each seat's transfer from the pink slips in its own state, drops starter cars, and hands both transfers to the directory with the result; the directory applies them to the two accounts, and the result message tells each seat what it kept and what it lost. The room also hands over the cars each seat raced, and the directory checks both sides against their accounts as it moves anything: a car moves only out of a collection that holds it, a keepsake stays with whoever holds it, and a side that raced a car its account does not own takes nothing, while whatever of its own it staked still goes to the other side. The room seats any legal garage, so this is where a garage its player never opened is caught. The client's toggle is consent only; the cars that move are the room's reading of the match.
