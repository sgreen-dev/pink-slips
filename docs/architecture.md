...
| `npm run social` | Draws the link-preview card, `public/social.jpg`, and the phone icon, `public/apple-touch-icon-ps.png`, from `scripts/social.html` and `scripts/icon.html` in a headless Chromium browser with the game's own fonts and colours. Run it again after changing the wordmark, the tagline or the palette |
