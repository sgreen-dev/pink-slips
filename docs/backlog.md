...
| Q5 | bug | high | Every collection writer discards `saveCollection`'s boolean, so a guest whose storage is blocked or full opens packs, scraps and buys, is shown all of it, and loses the lot on refresh with no message | `src/collection/persist.ts:116,140,154,176,187` | open |
| Q6 | gap | high | `Directory.scrap` and `Directory.buy` have no test, nor do their guest twins, though phase 30's "Done when" claims both sides behave the same | `src/server/directory.ts:394,405` | open |
| Q3 | risk | high | `server/worker.ts` is never type-checked: `server/tsconfig.json` is correct but referenced by nothing, so `tsc -b` never visits its 621 lines. Clean today | `tsconfig.json:3` | open |
| Q12 | risk | high | `counter/worker.js` is the only production code with no static checking of any kind — ESLint is scoped to `.ts`/`.tsx`, and it has no tsconfig and no tests | `eslint.config.js:12` | open |
| S4 | risk | high | `/me/cpu-result` takes the stakes transfer from the request body. Client trust is unavoidable for a browser-run match, but packs yield 1–2 random cards a minute where stakes yield three named Ultra Rares <sup>3</sup> | `src/server/directory.ts:477` | open |
| S5 | risk | med | The match seed is 32 bits and is the whole RNG state, so a player can brute-force it from their own opening hand and read the rest of the match. Rematch seeds are `seed + n` | `src/engine/rng.ts:6`, `src/server/room.ts:466` | open |
...
| Q20 | bug | med | Opening a pack does nothing at all when the network call fails — no spinner, no error. Every sibling action on that screen sets an error string | `src/ui/CollectionScreen.tsx:198` | open |
...
| G13 | gap | low | Buying refuses a card you already own, which `DESIGN.md` 12 does not say — so credits can never buy a second copy of a mod a deck needs | `src/collection/collection.ts:404` | open |
...
| S1 | risk | low | The counter's `Origin` check is not a control — any non-browser client sets the header. The one-per-10s-per-address limit is the real brake | `counter/worker.js:22` | open |
...
| D5 | gap | low | `docs/architecture.md` names two CI gaps; there are five, counting `scripts/*.ts`, `eslint.config.js` and the missing PR trigger | `docs/architecture.md:125` | open |
| D6 | gap | low | `README.md` calls `npm run build` a type-check without the caveat that the room worker is not covered | `README.md:39` | open |
...
3. Not fixable by removing trust — a CPU match runs entirely in the browser, so `won` is client-reported too. The fix is to bound what the stakes path can mint.
