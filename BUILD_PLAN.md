...
20. Music on phones starts on the first tap most of the time, but not every time (reported 2026-09-04). Needs a phone in hand. Look at: whether the failing taps are on a button or a scroll, since a touch that scrolls ends without a click; whether the audio engine reports `interrupted` rather than `suspended` after the phone was used for something else, which iPhones do and which a plain resume does not clear; whether the first track had finished decoding by the tap, which a slow connection delays; and whether the silent loop's media element started, which some phones refuse until a tap on a control. A small on-screen readout of the engine state and the decode state behind a query flag would settle it in one session
